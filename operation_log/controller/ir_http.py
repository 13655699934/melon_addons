...
class IrHttp(models.AbstractModel):
    ...
    def _monitoring_blacklist(cls, request):
        ...
        if path_info.endswith(('/write', '/create', '/unlink', '/web/export/xlsx', '/report/download', '/base_import/set_file')):
            return False
        return True
    ...
    def _monitoring_info(cls, request, response, begin, end):
        ...
        if path.endswith(('/web/export/xlsx')):
            info.update({
        ...
        try:
            ...
            if model_method == '/web/export/xlsx':
